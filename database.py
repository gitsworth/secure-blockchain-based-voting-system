...
def load_voters(db_path):
    ...
    if os.path.exists(db_path):
        try:
            ...
            voters_df = pd.read_csv(db_path, index_col='public_key')
            # Reset index to make public_key a column for easier filtering/checking
            voters_df.reset_index(inplace=True) 
        except Exception:
...
def update_voter_status(voters_df, public_key):
    ...
    row_index = voters_df[voters_df['public_key'] == public_key].index
    
    if not row_index.empty:
        # Update the 'has_voted' column
        voters_df.loc[row_index, 'has_voted'] = True
        return True
